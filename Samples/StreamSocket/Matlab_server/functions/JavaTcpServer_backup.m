...
switch(action)
    ...
    case 'accept'
        ...
        disp('Connected');
    case 'read'
        %data=zeros(1,100000,'int8'); tBytes=0;
        tstart=tic;
        %nBytes_test = zeros(1,10);
        %j = 1;
        %res = [];
        h=0; w=0;
        data=[]; tInt=0; flag=0;
        while(true)            
            ...
            nInt = TCP.inputStream.available*0.25;
            %for i = nInt, data(i) = DataInputStream(TCP.inputStream).readInt(); end;
            if(nInt ~= 0)
                if (flag == 0)
                    h = DataInputStream(TCP.inputStream).readInt();
                    w = DataInputStream(TCP.inputStream).readInt();
                    %nBytes = nBytes - 8;
                    nInt = nInt - 2;
                    flag = 1;
                end
                partdata = zeros(1,nInt,'int32');
                %for i = nBytes*0.25, partdata(i) = DataInputStream(TCP.inputStream).readInt; end;
                for i = nInt, partdata(i) = DataInputStream(TCP.inputStream).readInt(); end;
                data(tInt+1:tInt+nInt) = partdata;
